          ...
          <nav className="hidden items-center gap-6 md:flex" aria-label="Primary">
            {sectionsList.map((sec) => (
              <button 
                ...
                className={`text-[10px] font-mono font-bold uppercase tracking-widest transition-colors focus:outline-none ${
                  activeSection === sec.id ? "text-zinc-950" : "text-zinc-500 hover:text-zinc-950"
                }`}
              >
                {sec.label}
              </button>
            ...
            <div className="px-4 pt-2 pb-6 space-y-1">
              {sectionsList.map((sec) => (
                <button
                  key={sec.id}
                  onClick={() => scrollToSection(sec.id)}
                  className={`block w-full text-left py-3 px-3 text-xs font-mono font-bold uppercase tracking-wider border-b border-zinc-100 last:border-0 transition-colors ${
                    activeSection === sec.id ? "text-zinc-950 font-black" : "text-zinc-500 hover:text-zinc-950"
                  }`}
                >
                  {sec.label}
                </button>
